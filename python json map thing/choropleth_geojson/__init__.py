...
class choropleth():
    ...
    def __init__(self, apikey, df, geojson, glabel):
        ...
        self.n_ = len(self.geojson_['objects'])
    ...
    def reindex_df(self):
        ...
        """
        n = self.n_
        area_name = [self.geojson_['objects'][k]['properties'][self.glabel_] for k in range(n)]
    ...
    def get_centers(geojson):
        ...
        """
        lat_cen, lon_cen =[], []
        n = len(geojson['objects'])

        for k in range(n):
            geometry = geojson['objects'][k]['geometry']
            largest = 0
            largest_ind = 0
            ...
            #in the case of a multipolygon, take the largest polygon (largest in terms of having the most coordinates)   
                ...
                for j in range(len(geojson['objects'][k]['geometry']['coordinates'])):
                    poly_len = len(geojson['objects'][k]['geometry']['coordinates'][j][0])
    ...
    def make_sources(geojson, downsample = 0):
        ...
        for item in geojson_temp:
            ...
            sources.append(dict(type = 'objects',
                                features = [item])
                          )
        return sources
